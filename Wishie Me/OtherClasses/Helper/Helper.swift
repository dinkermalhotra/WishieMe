...
class Helper: NSObject {
    ...
    class func showLoader(onVC viewController: UIViewController) {
        SKActivityIndicator.spinnerColor(WishieMeColors.greenColor)
        
        SKActivityIndicator.spinnerStyle(.spinningFadeCircle)
        SKActivityIndicator.show("", userInteractionStatus: false)
    }
    
    class func showToast(onVC viewController: UIViewController) {
        Loaf(AlertMessages.NO_INTERNET, state: .custom(.init(backgroundColor: .black.withAlphaComponent(0.7))), location: .bottom, presentingDirection: .vertical, dismissingDirection: .vertical, sender: viewController).show()
    }
    ...
    class func showOKCancelAlertWithCompletion(onVC viewController: UIViewController, title: String, message: String, btnOkTitle: String, btnCancelTitle: String, onOk: @escaping ()->()) {
        DispatchQueue.main.async {
            ...
            alert.view.tintColor = WishieMeColors.greenColor
            alert.view.setNeedsLayout()
            viewController.present(alert, animated: true, completion: nil)
        }
    }
    ...
    class func showActionAlert(onVC viewController: UIViewController, isViewRequired: Bool = false, imageString: String? = nil, name: String? = nil, title: String?, titleOne: String, actionOne:@escaping ()->(), titleTwo: String, actionTwo:@escaping ()->(), styleOneType: UIAlertAction.Style, styleTwoType: UIAlertAction.Style) {
        DispatchQueue.main.async {
            let actionSheetControllerIOS8: UIAlertController = UIAlertController(title: title, message: nil, preferredStyle: .actionSheet)
            
            if isViewRequired {
                let view = UIView(frame: CGRect(x: 8.0, y: 8.0, width: actionSheetControllerIOS8.view.bounds.size.width - 8.0 * 4.5, height: 100.0))
                view.backgroundColor = UIColor.clear
                
                // setup image
                let imageView = UIImageView.init(frame: CGRect.init(x: (view.bounds.size.width / 2) - 35, y: 0, width: 70, height: 70))
                
                let block: SDExternalCompletionBlock? = {(image: UIImage?, error: Error?, cacheType: SDImageCacheType, imageURL: URL?) -> Void in
                    //print(image)
                    if (image == nil) {
                        imageView.image = Helper.birthdayImage(name ?? "")
                        return
                    }
                }
                ...
                imageView.layer.borderColor = WishieMeColors.greenColor.cgColor
                view.addSubview(imageView)
                
                // setup label
                let label = UILabel.init(frame: CGRect.init(x: view.bounds.origin.x, y: 74, width: view.bounds.size.width, height: 20))
                label.text = name
                label.textAlignment = .center
                label.font = WishieMeFonts.FONT_MONTSERRAT_MEDIUM_14
                label.textColor = .black
                view.addSubview(label)
                
                actionSheetControllerIOS8.view.addSubview(view)
            }
            ...
            let saveActionButton: UIAlertAction = UIAlertAction(title: titleOne, style: styleOneType) { action -> Void in
                actionOne()
            }
            actionSheetControllerIOS8.addAction(saveActionButton)
            
            let deleteActionButton: UIAlertAction = UIAlertAction(title: titleTwo, style: styleTwoType) { action -> Void in
                actionTwo()
            }
    ...
    class func showThreeWishieOptionActionAlert(onVC viewController: UIViewController, isViewRequired: Bool = false, imageString: String? = nil, name: String? = nil, title: String?, titleOne: String, actionOne:@escaping ()->(), titleTwo: String, actionTwo:@escaping ()->(), titleThree: String, actionThree:@escaping ()->(), actionCancel:@escaping ()->(), styleType: UIAlertAction.Style) {
        DispatchQueue.main.async {
            let actionSheetControllerIOS8: UIAlertController = UIAlertController(title: title, message: nil, preferredStyle: .actionSheet)
            ...
            let oneActionButton: UIAlertAction = UIAlertAction(title: titleOne, style: .default) { action -> Void in
                actionOne()
            }
            actionSheetControllerIOS8.addAction(oneActionButton)
            ...
            let threeActionButton: UIAlertAction = UIAlertAction(title: titleThree, style: styleType) { action -> Void in
                actionThree()
            }
            actionSheetControllerIOS8.addAction(threeActionButton)
    ...
    class func showFourOptionsActionAlert(onVC viewController: UIViewController, isViewRequired: Bool = false, imageString: String? = nil, name: String? = nil, title: String?, titleOne: String, actionOne:@escaping ()->(), titleTwo: String, actionTwo:@escaping ()->(), titleThree: String, actionThree:@escaping ()->(), titleFour: String, actionFour:@escaping ()->(), styleType: UIAlertAction.Style) {
        DispatchQueue.main.async {
            let actionSheetController: UIAlertController = UIAlertController(title: title, message: nil, preferredStyle: .actionSheet)
            
            if isViewRequired {
                let view = UIView(frame: CGRect(x: 8.0, y: 8.0, width: actionSheetController.view.bounds.size.width - 8.0 * 4.5, height: 100.0))
                view.backgroundColor = UIColor.clear
                ...
                actionSheetController.view.addSubview(view)
            }
            ...
            let actionButtonFour: UIAlertAction = UIAlertAction(title: titleFour, style: styleType) { action -> Void in
                actionFour()
            }
    ...
    class func thisWeek() -> [String] {
        let arrWeekDates = Date().getWeekDates()
        var dates = [String]()
        
        for i in 0..<arrWeekDates.thisWeek.count {
            let timeInterval = TimeInterval.init(-60)
            let date = arrWeekDates.thisWeek[i].addingTimeInterval(timeInterval)
            let dateFormatter = DateFormatter()
    ...
    class func nextWeek() -> [String] {
        let arrWeekDates = Date().getWeekDates()
        var dates = [String]()
        
        for i in 0..<arrWeekDates.nextWeek.count {
            let timeInterval = TimeInterval.init(-60)
            let date = arrWeekDates.nextWeek[i].addingTimeInterval(timeInterval)
            let dateFormatter = DateFormatter()
            dateFormatter.dateFormat = "MM-dd"
            dates.append(dateFormatter.string(from: date))
        }
